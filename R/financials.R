...
remove_outliers <- function(factor) {
  boxplot(factor)$stats
  max_fence <- boxplot(factor)$stats[5,1]
  min_fence <- boxplot(factor)$stats[1,1]
  fin$DivYield <- ifelse(factor < min_fence
                         | factor > max_fence, NA, factor)
  fin <- fin %>% filter(!is.na(factor))
  return(fin)
}

# remove outliers for dividend yield
fin <- remove_outliers(fin$DivYield) 
glimpse(fin)
fin <- remove_outliers(fin$Revenue_Growth)
glimpse(fin)
fin <- remove_outliers(fin$DPS_Growth)
fin <- remove_outliers(fin$ROE)
fin <- remove_outliers(fin$GPMargin)

glimpse(fin)
# factor = fin$DivYield
# boxplot(factor)$stats
# threshold <- outlier_constant * (boxplot(factor)$stats[4,1] - boxplot(factor)$stats[2,1])
# fin$DivYield <- ifelse(factor < boxplot(factor)$stats[2,1] - threshold 
#                        | factor > boxplot(factor)$stats[4,1] + threshold, NA, factor)
# fin <- fin %>% filter(!is.na(factor))
