...
def get_pbr_cummulative_returns(type: str):
  ...
  if type == "Bottom-Mid-High":
    return monthly_ret_cum[['Date'] + BOT_MID_HIGH]
  if type == "Quintiles":
    return monthly_ret_cum[['Date'] + QUINTILES]
  if type == "Deciles":
    return monthly_ret_cum[['Date'] + DECILES]

def pbr_factor_stat(type: str):
    monthly_ret = web.DataReader("Portfolios_Formed_on_BE-ME", "famafrench", start="1900-01-01")[0]
    n = len(monthly_ret)
    ...
    if type == "Bottom-Mid-High":
      return stat[['Columns'] + BOT_MID_HIGH]
    if type == "Quintiles":
      return stat[['Columns'] + QUINTILES]
    if type == "Deciles":
      return stat[['Columns'] + DECILES]