...
def get_size_cummulative_returns():
    monthly_ret = pd.read_csv(
        "https://github.com/eshinhw/quant-portfolio-visualizer/blob/main/src/famafrench/Portfolios_Formed_on_ME.CSV?raw=true",
    )
    ...
    monthly_ret_cum.rename(columns={"Lo 20": "SMALL", "Hi 20": "BIG"}, inplace=True)
    # print(monthly_ret_cum)
    return monthly_ret_cum[["Date", "SMALL", "BIG"]]


def size_factor_stat():
    data = {}

    monthly_ret = get_size_cummulative_returns()
    monthly_ret.set_index("Date", inplace=True)
    n = len(monthly_ret)
